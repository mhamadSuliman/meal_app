    ...
    <style>
        ...
        .meal-card-v2:hover {
            ...
            box-shadow:
                ...
                0 0 50px -10px rgba(251,146,60,.35);
        }
        .meal-img-wrap {
            position: relative;
            width: 100%;
            height: 240px;
            overflow: hidden;
            background: linear-gradient(135deg, #451a03, #1c0d06);
        }
        .meal-img {
        ...
        .meal-img-placeholder {
            ...
            display: flex; align-items: center; justify-content: center;
            background:
                radial-gradient(circle at 30% 30%, rgba(251,146,60,.3), transparent 60%),
                linear-gradient(135deg, #7c2d12, #431407);
            color: rgba(254,243,199,.45);
            font-weight: 900;
            position: relative;
        }
        .meal-img-placeholder::after {
            content: '🍽️';
            position: absolute;
        ...
        .meal-card-v2:hover .meal-num-float {
        ...
        .meal-price-float {
            ...
            border-radius: 12px;
            box-shadow:
                0 10px 25px -8px rgba(251,146,60,.6),
                inset 0 1px 0 rgba(255,255,255,.4);
            display: inline-flex; align-items: baseline;
            transition: transform .4s;
            z-index: 2;
        }
        .meal-card-v2:hover .meal-price-float {
            transform: scale(1.08) translateY(-2px);
        }
        .meal-type-chip {
            position: absolute;
            top: 16px; left: 16px;
            background: rgba(12,6,4,.75);
        ...
        <div class="fade-up" style="animation-delay:.2s">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($meals as $i => $meal)
                        @php
                            ...
                            $hasImage = !empty($meal->image);
                            $imgUrl = $hasImage ? asset('storage/' . $meal->image) : null;
                        @endphp

                        <div class="meal-card-v2 group">
                            {{-- IMAGE / PLACEHOLDER --}}
                            <div class="meal-img-wrap">
                                @if($hasImage)
                                    <img src="{{ $imgUrl }}" alt="{{ $meal->name }}" class="meal-img">
                                @else
                                    <div class="meal-img-placeholder">
                                        <span class="serif text-7xl italic">{{ mb_substr($meal->name, 0, 1) }}</span>
                                    </div>
                                @endif

                                <div class="meal-overlay"></div>

                                <span class="meal-num-float serif">{{ $num }}</span>

                                <div class="meal-price-float">
                                    <span class="serif font-black text-2xl">{{ $meal->price }}</span>
                                    <span class="text-[10px] font-bold opacity-80 mr-1">ل.س</span>
                                </div>

                                @if(!empty($meal->type))
                                    <span class="meal-type-chip">{{ $meal->type }}</span>
                                @endif
                            </div>
                            ...
                            <div class="meal-body">
                                <div class="flex items-center gap-2 mb-2">
                                    <span class="w-4 h-px bg-orange-400/60"></span>
                                    <span class="text-orange-400 text-[10px] font-black tracking-widest">DISH #{{ str_pad($meal->id, 3, '0', STR_PAD_LEFT) }}</span>
                                </div>

                                <h3 class="serif text-xl font-black mb-2 leading-tight" style="color:#fef3c7">
                                    {{ $meal->name }}
                                </h3>

                                @if(!empty($meal->description))
                                    <p class="text-amber-200/60 text-sm italic serif mb-4 line-clamp-2">
                                        {{ $meal->description }}
                                    </p>
                                @endif

                                <form method="POST" action="/owner/meals/{{ $meal->id }}"
                                      onsubmit="return confirm('هل أنت متأكد من حذف هذه الوجبة؟');">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn-delete w-full justify-center" data-testid="delete-meal-{{ $meal->id }}">
            ...
            @else
                <div class="menu-card p-12 text-center">
                    ...
                    <a href="{{ route('owner.meals.create') }}" class="btn-add">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"/></svg>
                        أضف أول وجبة
                    </a>
                </div>
